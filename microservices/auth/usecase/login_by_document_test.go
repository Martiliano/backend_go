...
//
// microservices => account => usecase => login_by_document_test.go
//
...
func TestLoginByDocument_Success(t *testing.T) {
	...
	if err != nil {
		t.Error("Login By Document, Expected JWT Token, Got :", err)
		return
	}
	...
	_, err = usecase.LoginByDocument("12345678901", passwordForValidate)
...
func TestLoginByDocument_IncorrectPassword(t *testing.T) {
	...
	token, err := usecase.LoginByDocument("12345678901", passwordForValidate)

	if err == nil {
		t.Error("Login By Document, Expected ERROR, Got :", token)
		return
	}

	if err != nil && !strings.HasPrefix(err.Error(), "crypto/bcrypt: hashedPassword is not the hash of the given password") {
		t.Error("Login By Document, Expected ERROR 'crypto/bcrypt: hashedPassword is not the hash of the given password', Got :", err)
		return
	}
	//
}
...
func TestLoginByDocument_DocumentNotExists(t *testing.T) {
	...
	if err != nil && !strings.HasPrefix(err.Error(), "Não existe a Account referida pelo Document") {
		t.Error("Login By Document, Expected ERROR 'Não existe a Account referida pelo Document', Got :", err)
		return
	}
