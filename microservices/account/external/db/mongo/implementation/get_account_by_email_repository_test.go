...
func TestGetAccountByEmailRepository(t *testing.T) {
	...
	if err != nil {
		t.Error("GetAccountByEmail Account, Error Create Test Account :", err)
	}

	result, err := repository.GetAccountByEmailRepository(a.Email)
	...
	if id != result.Id {
		t.Error("GetAccountByEmail Account, Expected :", id, ", Got : ", result.Id)
	}
}
