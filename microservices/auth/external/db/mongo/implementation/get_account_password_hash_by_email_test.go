...
//
// microservices => auth => external => db => mongo => implementation => get_account_password_hash_by_email_test.go
//
...
func TestGetAccountPasswordHashByEmailRepository_Success(t *testing.T) {
	...
	if err != nil {
		t.Error("Get Account PasswordHash By Email, Expected  PasswordHash, Got :", err)
	}
	...
	_, err = repository_auth.GetAccountPasswordHashByEmailRepository(a.Email)
...
func TestGetAccountPasswordHashByEmailRepository_InvalidHashPassword(t *testing.T) {
	ctx := context.Background()
	...
	if err != nil && !strings.HasPrefix(err.Error(), msg) {
		t.Error("Get Account PasswordHash By Email, Expected ' ", msg, " ', Got :", err)
	}
}
