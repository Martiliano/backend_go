...
//
// microservices => auth => presenter => grpc => v1 => presentation => login_by_phone_test.go
//
//
...
func TestLoginByPhone_Success(t *testing.T) {
	...
	if err != nil {
		t.Error("Login By Email, Expected  PasswordHash, Got :", err)
	}
	...
	_, err = cli.LoginByPhone(ctx, &pb.LoginByPhoneRequest{Phone: a.PhoneNumber, Password: password})
...
func TestLoginByPhone_PhoneNotExists(t *testing.T) {
	...
	if err != nil {
		t.Error("Login By Email, Expected  PasswordHash, Got (4) :", err)
	}

	fmt.Println("TestLoginByPhone_DocumentNotExists id (5) : ", id)
	...
	if err != nil {
		t.Error("Login By Email, Expected  PasswordHash, Got (6) :", err)
	}

	_, err = account_repository_mongo.SetPasswordRepository(id, string(hashPassord))

	if err != nil {
		t.Error("Login By Email, Expected  PasswordHash, Got (7) :", err)
	}

	msg := "rpc error: code = Unknown desc = mongo: no documents in result"

	_, err = cli.LoginByPhone(ctx, &pb.LoginByPhoneRequest{Phone: "11988776655", Password: password})

	if err != nil && !strings.HasPrefix(err.Error(), msg) {
		t.Error("Login By Email, Expected  PasswordHash, Got (8) :", err)
	}
...
func TestLoginByPhone_IncorrectPassword(t *testing.T) {
	...
	_, err = cli.LoginByPhone(ctx, &pb.LoginByPhoneRequest{Phone: a.PhoneNumber, Password: "IncorrectPassword"})
