...
//
// microservices => account => external => db => mongo => implementation => get_account_by_recovery_token_repository.go
//
...
func (sMR *accountMongoRepository) GetAccountByRecoveryTokenRepository(token string) (*account.Account, error) {
	collection := sMR.dbServer.Database(sMR.dbName).Collection(collectionName)

	result := account_model.AccountDataStruct{}
	a := account.Account{}

	filter := bson.D{primitive.E{Key: "token_password_recovery", Value: token}}
