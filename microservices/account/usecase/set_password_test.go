...
//
// microservices => account => usecase => set_password_test.go
//

func TestSetPassword_PasswordNotMatch(t *testing.T) {
	...
	token, err := jwtManager.GenerateWithDetails("62956508ccde2d63320a8a59", []string{"Desenvolvedor"},
		[]string{"Desevolvedor", "Administrador"}, "backend_go.v1.setpassword", time.Now().Unix(),
		time.Now().Add(time.Millisecond*200).Unix())

	password := "strongPassword"
	confirmPassword := "strongPasswordError"

	err = usecase.SetPassword(token, password, confirmPassword)
	msg := "A senha e a confirmação são diferentes"

	if err == nil {
		t.Error("SetPassword, Expected error message: '", msg, "', Got:", err)
...
func TestSetPassword_InvalidIssuer(t *testing.T) {
...
func TestSetPassword_TokenNotFound(t *testing.T) {
...
func TestSetPassword_ErrorUpdatePassword(t *testing.T) {
	...
	err = usecase.SetPassword(token, password, confirmPassword)
	msg := "Não foi possivel setar a senha para o Id"
...
func TestSetPassword_Success(t *testing.T) {
	...
	if err != nil {
		t.Error("SetPassword, Expected SUCCESS, Got:", err)
	}
}
