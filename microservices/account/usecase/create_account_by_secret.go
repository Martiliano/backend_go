...
//
// microservices => account => usecase => create_account_by_secret.go
//
...
func (aUC *AccountUseCase) CreateAccountBySecret(a account.Account, secret string) (string, error) {

	if secret != aUC.serverSecret {
		return "", fmt.Errorf("Palavra secreta Invalida")
	}
