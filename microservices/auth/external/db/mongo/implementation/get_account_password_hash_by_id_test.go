...
//
// microservices => auth => external => db => mongo => implementation => get_account_password_hash_by_id_test.go
//
...
func TestGetAccountPasswordHashByIdRepository_Success(t *testing.T) {
	...
	if err != nil {
		t.Error("Get Account PasswordHash By Id, Expected  PasswordHash, Got :", err)
	}
	...
	_, err = repository_auth.GetAccountPasswordHashByIdRepository(id)
...
func TestGetAccountPasswordHashByIdRepository_InvalidHashPassword(t *testing.T) {
	ctx := context.Background()
	...
	if err != nil && !strings.HasPrefix(err.Error(), msg) {
		t.Error("Get Account PasswordHash By Id, Expected ' ", msg, " ', Got :", err)
	}
}
